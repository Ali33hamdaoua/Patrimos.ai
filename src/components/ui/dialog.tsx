...
/**
 * Dialog utilitaire (shadcn-style, Radix sous le capot). Pas affiché sur le
 * site — prêt à servir pour la modale "Demander une démo" plus tard.
 *
 * Adaptations Patrimos :
 *   - focus-visible:ring : passé de #6B97FF (bleu) à #C9A55C (or antique).
 *   - variant `destructive` : bg-red-500 → bg-[#7a2e26] (bordeaux désaturé,
 *     respecte la règle "aucune couleur vive").
 * Les tokens sémantiques utilisés (bg-card, text-foreground, border-border,
 * bg-muted, text-muted-foreground) sont définis dans tailwind.config.ts.
 */
...
const buttonVariants = cva(
  [
    ...
    "focus-visible:ring-1 focus-visible:ring-[#C9A55C]",
  ],
  {
    variants: {
      variant: {
        ...
        destructive:
          "bg-[#7a2e26] text-white hover:bg-[#7a2e26]/90",
      },
      size: {
