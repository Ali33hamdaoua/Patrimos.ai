...
type UniformDef = { value: UniformValue; type: UniformType };
type Uniforms = Record<string, UniformDef>;

type PreparedUniform = { value: unknown; type: string };

/* -------------------------------------------------------------------------- */
/*  CanvasRevealEffect : wrapper public.                                      */
/* -------------------------------------------------------------------------- */
    ...
    <div className={cn("h-full relative w-full", containerClassName)}>
      <div className="h-full w-full">
        <DotMatrix
          ...
          dotSize={dotSize ?? 3}
          opacities={opacities}
          // Les flags `reverse` et `animationSpeed` sont passés via le shader
          // string, comme dans la source d'origine.
          shader={`
...
/* -------------------------------------------------------------------------- */
/*  DotMatrix : construit les uniforms + le fragment shader.                  */
/* -------------------------------------------------------------------------- */
...
/* -------------------------------------------------------------------------- */
/*  ShaderPlane : mesh + ShaderMaterial Three.js.                             */
/*  Renommé pour ne pas shadow `THREE.ShaderMaterial`.                        */
/* -------------------------------------------------------------------------- */
...
const ShaderPlane = ({
  ...
  const material = useMemo(() => {
    const prepared: Record<string, PreparedUniform> = {};
    ...
    return new THREE.ShaderMaterial({
      ...
      uniforms: prepared as unknown as Record<string, THREE.IUniform>,
...
const Shader: React.FC<{ source: string; uniforms: Uniforms }> = ({
  source,
  uniforms,
}) => {
  return (
